...
class Terminal(models.Model):
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="terminals"
    )
...
class OutboxRecord(models.Model):  # terminal side — the queue
    ...
    entity = models.CharField(max_length=30)  # "sale" | "stock_adjust" | "userlog"
...
class CatalogVersion(models.Model):  # per-org catalog counter
    organization = models.OneToOneField(Organization, on_delete=models.CASCADE)
    value = models.BigIntegerField(default=0)


class SyncState(models.Model):  # terminal bookkeeping
