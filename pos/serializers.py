...
from inventory.models import Bundle, Item

from .models import CartItem, Customer, Sale
...
class SaleItemField(serializers.Field):
    ...
    def to_internal_value(self, data):
        ...
        if quantity is None or (
            not isinstance(quantity, int) and not isinstance(quantity, str)
        ):
        ...
        if item_id is not None and bundle_id is not None:
            raise serializers.ValidationError(
                "Provide either item_id or bundle_id, not both."
            )
        ...
        if organization is not None:
            item_qs = item_qs.filter(organization=organization)
            bundle_qs = bundle_qs.filter(organization=organization)

        if item_id is not None:
            try:
                item = item_qs.get(pk=item_id)
            except Item.DoesNotExist:
                raise serializers.ValidationError(f"Item {item_id} not found.")
            return {"type": "item", "item": item, "quantity": quantity}
        try:
            bundle = bundle_qs.get(pk=bundle_id)
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def validate_customer(self, customer):
        if customer is None:
            return customer
        organization = self.context.get("organization")
        if organization is not None and customer.organization_id != organization.id:
            raise serializers.ValidationError("Customer not found in your organization.")
        return customer

    def create(self, validated_data):
        items_data = validated_data.pop("items")
        organization = validated_data.pop("organization", None) or self.context.get(
            "organization"
        )
        ...
        sale = Sale.objects.create(organization=organization, **validated_data)
        total = Decimal("0")
